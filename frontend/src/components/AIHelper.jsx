...
import { Send, X, Minimize2, Loader2, MessageSquare, Sparkles } from 'lucide-react';
...
const AIHelper = () => {
    ...
    const [messages, setMessages] = useState([
        {
            role: 'model',
            text: "Hi! I'm Chintu, your friendly Bill Splitter guide! 👋 I can help you manage your rooms, split bills with ease, and much more. How can I assist you today?"
        }
    ]);
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end font-inter">
            {/* Chat Window */}
            {isOpen && (
                <div className="mb-4 w-[350px] md:w-[400px] glass-morphism rounded-3xl shadow-2xl overflow-hidden flex flex-col transition-all duration-500 animate-scale-up border-none">
                    {/* Header */}
                        ...
                        <div className="flex items-center gap-3">
                            <div className="relative">
                                <div className="w-12 h-12 rounded-full bg-white/30 backdrop-blur-md flex items-center justify-center border border-white/40 shadow-inner">
                                    <img src="/ai-mascot.png" alt="Chintu" className="w-10 h-10 object-contain drop-shadow-md" />
                                </div>
                    ...
                    {/* Messages Area */}
                    <div className="h-[450px] overflow-y-auto p-5 space-y-4 bg-transparent scrollbar-hide">
                        {messages.map((msg, index) => (
                            ...
                            >
                                <div
                                    className={`max-w-[85%] px-4 py-3 rounded-2xl text-[14px] leading-relaxed shadow-sm transition-all hover:shadow-md ${msg.role === 'user'
                        ...
                        <div className="px-5 pb-2 flex gap-2 overflow-x-auto scrollbar-hide">
                            {suggestedQuestions.map((q, i) => (
                                <button
                                    ...
                                    className="whitespace-nowrap px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-300 text-xs font-bold rounded-full border border-blue-100 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"
                                >
                                    {q}
                                </button>
                            ))}
            ...
            <div className="relative">
                {!isOpen && (
                    <>
                        <div className="absolute inset-0 rounded-full bg-blue-500/30 animate-ripple"></div>
                        <div className="absolute inset-0 rounded-full bg-blue-500/20 animate-ripple [animation-delay:0.7s]"></div>
                    </>
                )}
                <button
                    onClick={() => setIsOpen(!isOpen)}
                    className={`group relative flex items-center justify-center w-16 h-16 rounded-full shadow-2xl transition-all duration-500 hover:scale-110 active:scale-95 ${isOpen
                            ? 'bg-gray-900 text-white transform rotate-90'
                ...
                >
                    {isOpen ? (
                        ...
                        <div className="relative">
                            <img src="/ai-mascot.png" alt="Chintu" className="w-12 h-12 object-contain drop-shadow-xl" />
